...
class AnimeBot:
    ...
    def get_user_session(self, user_id: int, message_id: int = None):
        ...
        if message_id:
            return self.user_sessions.get(message_id, {}).get('user_data', {})
        return self.user_sessions.get(user_id, {})

    def set_user_session(self, user_id: int, data: dict, message_id: int = None):
        """Set user session data"""
        if message_id:
            if message_id not in self.user_sessions:
                self.user_sessions[message_id] = {}
            self.user_sessions[message_id].update({
                'user_data': data,
    ...
    def clear_user_session(self, user_id: int, message_id: int = None):
        """Clear user session"""
        if message_id and message_id in self.user_sessions:
            del self.user_sessions[message_id]
    ...
    async def show_anime_details(self, client: Client, callback_query: CallbackQuery, anime_id: int):
        try:
            ...
            # Add Delete Episodes button for admins
    ...
    async def clear_expired_sessions(self, client: Client):
        now = datetime.now()
        expired = []
        ...
        for user_id in expired:
            session = self.user_sessions.get(user_id)
            # Try to delete the message if available
            try:
            ...
            del self.user_sessions[user_id]
            logger.info(f"Session expired and cleared for user {user_id}")
